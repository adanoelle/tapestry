...
mod tests {
    ...
    fn test_commit_validation() {
        ...
        let valid = service.validate_commit_message("feat: add new feature").unwrap();
        assert!(valid);
        
        // Invalid commit
        let invalid = service.validate_commit_message("Added new feature").unwrap();
        assert!(!invalid);
        
        // Empty message
        let empty = service.validate_commit_message("").unwrap();
        assert!(!empty);
    }
